  ...
  return (
    <div className="cart" >
      <h1 className="title-cart">Productos en el carrito</h1>
      {
        cart.map((productCart) => (
          <div className="item-cart" key={productCart.id}>
            ...
            <p className="text-item-cart"> Subtotal: ${productCart.precio * productCart.quantity} </p>
            <button onClick={ ()=> deleteProductById(productCart.id) } className="btn-delete" > eliminar</button>
          </div>
        ))
      }
      ...
      <div className="info-cart">
        ...
        <button onClick={deleteCart} className="vaciar-carro" > Vaciar carrito</button>
      </div>
    </div>
  )
}
